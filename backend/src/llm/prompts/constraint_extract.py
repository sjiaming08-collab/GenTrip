...
8. return_by 格式为 HH:MM（24 小时制）；可与 time_budget_minutes 同时存在。
9. budget_per_person、time_budget_minutes、poi_count 必须是正整数；缺失时在 assumptions 说明来源。
10. assumptions 每项包含 slot、assumed_value、message；仅记录推断/默认/沿用记忆值，用户已明确表述的不写入 assumptions。
11. 用户提到菜系或餐饮类型时，必须填写 preferred_cuisines（数组），填标准词不展开：
