...
The current gate has 8 conversations and 16 turns, plus 22 declarative constraint-language variation cases in `backend/fixtures/golden_constraint_cases.json`. The expanded language set covers morning/noon/night start times, minute and half-day durations, cuisine aliases, queue wording, shopping intent, green-space exclusion, and memory inheritance of cuisine, domains, and queue tolerance. Conversation coverage includes cold plans with district, budget, start/return time, duration, queue and exclusion constraints; Plan/Replan transitions; add, replace, category delete, ordinal delete, budget override, empty routes, memory inheritance, and non-travel rejection. The `huangpu_cold_exclusions` case is intentionally a semantic-only case because it exercises automatic constraint relaxation; it must preserve exclusions but is not accepted as a quality reference route.
