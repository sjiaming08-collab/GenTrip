...
const liveEvents = computed<SSEProgressEvent[]>(() => {
  if (props.events.length) return normalizeRuntimeEvents(props.events)
  return normalizeRuntimeEvents((props.route?.meta.phase_log ?? []).map((entry) => ({
...
const policySummary = computed(() => ({
  active: props.route?.meta.active_policies?.length ?? 0,
  dropped: props.route?.meta.dropped_policies?.length ?? 0,
  failures: props.route?.meta.planning_failures?.length ?? 0,
  repairs: props.route?.meta.repair_actions?.length ?? 0,
}))
...
async function inviteMember() {
  ...
  if (!email) return
  busy.value = true
  try { await addTenantMember(email, memberRole.value); memberEmail.value = ''; message.value = '成员已加入当前旅行空间。'; await refreshActiveTab() } catch { loadError.value = '添加成员失败。' } finally { busy.value = false }
}
...
<template>
  <Teleport to="body">
    <div v-if="open" class="console-layer" @click.self="emit('close')">
      <section class="runtime-console" aria-label="运行与协作管理">
        <header class="console-header">
          <div><p class="eyebrow">GENTRIP · 运行中心</p><strong>{{ identity?.tenant.name || '本地规划环境' }}</strong></div>
        ...
        <nav class="console-tabs" aria-label="控制台分区">
          ...
          <button :class="{ active: activeTab === 'workspace' }" type="button" :disabled="!identity" @click="selectTab('workspace')">协作管理</button>
        </nav>
        ...
        <div v-if="activeTab === 'runtime'" class="console-content">
          <div class="run-overview">
            <div class="overview-line"><span>当前任务</span><b :class="{ online: llmEnabled }">{{ llmEnabled ? 'LLM 在线' : '规则降级' }}</b></div>
          ...
          <section class="console-section">
            ...
            <ol v-if="displayedEvents.length" class="phase-list">
              <li v-for="(event, index) in displayedEvents" :key="runtimeEventKey(event, index)" :class="[event.status || 'running', { current: loading && index === displayedEvents.length - 1 }]">
                <span class="phase-node"><RuntimePhaseIcon :phase="event.phase" :size="16" /></span><div class="phase-copy"><div class="phase-title"><strong>{{ presentPhase(event.phase).title }}</strong><small>{{ presentStatus(event.status) }}</small></div><p>{{ event.status === 'running' ? presentPhase(event.phase).description : stageOutcome(event) }}</p><div v-if="details(event).length" class="event-details"><span v-for="([key, value]) in details(event)" :key="key"><b>{{ key }}</b>{{ displayValue(value) }}</span></div></div>
              </li>
            </ol><p v-else class="empty-state">提交任务后，这里会实时显示每个节点的输出。</p>
          </section>

          <section class="console-section"><div class="section-heading"><h3>LLM 调用</h3><span>{{ liveLlmCalls.length }} 次</span></div>
          ...
          <section class="console-section"><div class="section-heading"><h3>工具与降级</h3><span>{{ liveToolCalls.length }} 次</span></div>
            ...
            <p v-if="policySummary.active || policySummary.dropped || policySummary.failures || policySummary.repairs" class="policy-summary">策略：启用 {{ policySummary.active }}，撤销 {{ policySummary.dropped }} · 可行性冲突 {{ policySummary.failures }} · 定向修复 {{ policySummary.repairs }}</p>
          </section>
        </div>
...
.runtime-console{width:min(560px,100vw);height:100%;overflow:auto;border-left:1px solid #d8dfda;border-radius:16px 0 0 16px;background:#fbfcfb;color:#283730;box-shadow:-20px 0 50px rgba(28,42,35,.18)}
.console-header{display:flex;align-items:center;justify-content:space-between;padding:24px 26px 19px;border-bottom:1px solid #dbe9df}.eyebrow{margin:0 0 7px;color:#2c956c;font:700 10px/1.2 "Microsoft YaHei","微软雅黑",sans-serif}.console-header strong{font:700 22px "Microsoft YaHei","微软雅黑",sans-serif}.icon-button{width:30px;height:30px;border:1px solid #c5dbcc;border-radius:50%;background:#fff;color:#286b4c;font-size:21px;line-height:1;cursor:pointer}
...
.run-overview{display:grid;gap:8px;padding:15px;border:1px solid #cfe5d6;background:#f3faf5;border-radius:9px}.overview-line{display:flex;justify-content:space-between;color:#277452;font:700 10px "Microsoft YaHei","微软雅黑",sans-serif}.overview-line b{color:#9b7627}.overview-line b.online{color:#1c9562}.run-overview code{overflow:hidden;color:#1e6648;font:12px "Microsoft YaHei","微软雅黑",sans-serif;text-overflow:ellipsis;white-space:nowrap}.run-overview small{color:#789087;font-size:11px}.metric-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}.metric-grid div{padding:11px;border:1px solid #e0ece3;border-radius:7px;background:#fff}.metric-grid span,.metric-grid strong{display:block}.metric-grid span{color:#789087;font-size:11px}.metric-grid strong{margin-top:5px;color:#1d563d;font-size:19px}
.console-section{margin-top:26px}.section-heading{display:flex;align-items:center;justify-content:space-between;gap:10px}.section-heading h3{margin:0;font-size:14px}.section-heading>span{color:#80948a;font:10px "Microsoft YaHei","微软雅黑",sans-serif}.phase-list{position:relative;display:grid;gap:0;margin:14px 0 0;padding:0;list-style:none}.phase-list:before{position:absolute;top:17px;bottom:24px;left:15px;width:1px;background:#dddeda;content:""}.phase-list li{position:relative;display:flex;gap:12px;min-height:52px;padding:0 0 12px}.phase-node{z-index:1;display:grid;width:32px;height:32px;flex:0 0 auto;place-items:center;border:1px solid #deded8;border-radius:8px;background:#fbfdfb;color:#767873}.phase-list li.completed .phase-node,.phase-list li.success .phase-node{color:#3f7b62}.phase-list li.failed .phase-node{border-color:#e3b8b0;color:#a54d3f}.phase-list li.current .phase-node{border-color:#dfb39f;background:#f8ede8;color:#bd5d3a;box-shadow:0 0 0 3px rgba(189,93,58,.08)}.phase-copy{min-width:0;flex:1;padding:3px 0 8px}.phase-list li.current .phase-copy{margin-top:-4px;padding:8px 10px;border:1px solid #ead1c7;border-radius:8px;background:#fbf5f1}.phase-title{display:flex;justify-content:space-between;gap:10px}.phase-title strong{color:#353733;font-size:12px;font-weight:650}.phase-title small{color:#858781;font-size:10px}.phase-list li.completed .phase-title small,.phase-list li.success .phase-title small{color:#3f7b62}.phase-list li.current .phase-title small{color:#ad5436}.phase-copy p{margin:5px 0 0;color:#6e716b;font-size:12px;line-height:1.5}.event-details{display:flex;flex-wrap:wrap;gap:5px;margin-top:8px}.event-details span{max-width:100%;padding:3px 6px;border-radius:4px;background:#f0f1ee;color:#646b65;font:10px "Microsoft YaHei","微软雅黑",sans-serif;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.event-details b{margin-right:4px;color:#4f6e60}.call-list,.tool-list{display:grid;gap:8px;margin-top:12px}.call-list article,.tool-list article{padding:10px 11px;border:1px solid #e0ece3;border-radius:7px;background:#fff}.call-list div{display:flex;justify-content:space-between;gap:10px}.call-list span{font:10px "Microsoft YaHei","微软雅黑",sans-serif}.call-list .success{color:#25865b}.call-list .failed{color:#b34d45}.call-list .fallback,.call-list .skipped{color:#a07a26}.call-list small,.tool-list small,.row small,.audit-row small{display:block;margin-top:5px;color:#7a9084;font-size:11px}.tool-list article{display:flex;justify-content:space-between;gap:10px}.tool-list strong{font-size:12px}.empty-state,.tool-summary,.degraded-summary,.policy-summary{margin:11px 0 0;color:#7b9185;font-size:12px;line-height:1.6}.degraded-summary{color:#9b7522}.policy-summary{color:#4f6e60}.text-button,.member-form button{border:1px solid #bfdcc9;border-radius:5px;background:#fff;color:#28704f;padding:6px 9px;font-size:11px;cursor:pointer}.row{display:flex;align-items:center;gap:10px;padding:13px 0;border-bottom:1px solid #e4eee7}.row>div{min-width:0;flex:1}.row strong{display:block;overflow:hidden;font-size:13px;text-overflow:ellipsis;white-space:nowrap}.member-form{display:grid;grid-template-columns:minmax(0,1fr) 84px 54px;gap:7px;margin:13px 0 4px}.member-form input,.member-form select,.row select{min-width:0;padding:7px;border:1px solid #cfe2d5;border-radius:5px;background:#fff;color:#28523e;font-size:11px}.member-form button{padding:7px}.audit-row{padding:10px 0;border-bottom:1px solid #e4eee7}.audit-row strong{font-size:12px}
...
.runtime-console, .runtime-console * { font-family: "Microsoft YaHei", "微软雅黑", sans-serif; }
.console-header strong { font-size: 21px; font-weight: 700; }
.run-overview, .metric-grid div, .phase-node, .phase-list li.current .phase-copy, .call-list article, .tool-list article { border-radius: 11px; }
.notice, .text-button, .member-form button, .member-form input, .member-form select, .row select { border-radius: 9px; }
.eyebrow, .overview-line, .run-overview code, .section-heading > span, .event-details span, .call-list span { font-family: "Microsoft YaHei", "微软雅黑", sans-serif; }
</style>
