...
const PHASES: Record<string, PhasePresentation> = {
  ...
  constraint_compile: { title: '编译约束合同', description: '正在区分硬约束、软偏好与默认策略。' },
  ...
  activity_blueprint: { title: '构思活动蓝图', description: '正在生成均衡型与体验型活动顺序。' },
  blueprint_compile: { title: '预检蓝图时间', description: '正在检验活动窗口，并移除冲突的可选项。' },
  ...
  failure_directed_repair: { title: '定向修复路线', description: '正在根据结构化失败原因进行局部修复。' },
...
export function nextPhase(event: SSEProgressEvent | undefined): string | null {
  ...
  const next: Record<string, string | undefined> = replan
    ...
    : {
        turn_orchestrate: 'constraint_extract', constraint_extract: 'constraint_compile', constraint_compile: 'planning_decision', planning_decision: 'route_bundle_search',
        route_bundle_search: 'geo_resolve', geo_resolve: 'activity_blueprint', activity_blueprint: 'blueprint_compile', blueprint_compile: 'poi_retrieve', poi_retrieve: 'route_generate',
